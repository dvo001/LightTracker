...
def list_tags(request: Request):
    # read from running engine via request to avoid circular imports
    engine = getattr(request.app.state, 'tracking_engine', None)
    if not engine:
        return {"tags": []}
    items = []
...
def get_position(request: Request, tag_mac: str):
    engine = getattr(request.app.state, 'tracking_engine', None)
    if not engine:
