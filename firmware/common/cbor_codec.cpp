...
static bool cbor_read_len(CborView& cv, uint8_t ai, uint64_t& len){
  ...
  if (ai == 24){ if (cv.p >= cv.end) return false; len = *cv.p++; return true; }
  if (ai == 25){ if (cv.p+1 >= cv.end) return false; len = (cv.p[0]<<8)|cv.p[1]; cv.p+=2; return true; }
  if (ai == 26){ if (cv.p+3 >= cv.end) return false; len = ((uint32_t)cv.p[0]<<24)|((uint32_t)cv.p[1]<<16)|((uint32_t)cv.p[2]<<8)|cv.p[3]; cv.p+=4; return true; }
  return false;
}
  ...
  switch (maj){
    ...
    case 4: { // array
      ...
      for (uint64_t i=0;i<val;i++){
        DynamicJsonDocument tmp(128);
        if (!cbor_to_json(cv, tmp.to<JsonVariant>())) return false;
    ...
    case 5: { // map
      ...
      for (uint64_t i=0;i<val;i++){
        ...
        if (cv.p + klen > cv.end) return false;
        String key;
        key.reserve(klen);
